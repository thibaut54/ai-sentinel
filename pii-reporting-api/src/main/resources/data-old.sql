...
-- GLINER PII TYPES
-- ============================================================================
-- All detector_label values MUST belong to the official NVIDIA GLiNER PII
-- taxonomy (snake_case). Reference list :
--   name, first_name, last_name, email, phone_number, fax_number, address,
--   street_address, city, state, postcode, country, ssn, national_id, tax_id,
--   certificate_license_number, credit_debit_card, cvv, pin, account_number,
--   bank_routing_number, swift_bic, iban, url, ipv4, ipv6, mac_address,
--   api_key, user_name, password, http_cookie, device_identifier,
--   vehicle_identifier, license_plate, medical_record_number,
...
-- For internal pii_types without a direct NVIDIA equivalent (SALARY,
-- DIAGNOSIS, MEDICATION, CRIMINAL_RECORD, AVS_NUMBER, PATIENT_ID, ...),
-- we map to the closest official label and document the compromise inline.
-- ============================================================================
...
VALUES
    ...
    -- PASSPORT_NUMBER / DRIVER_LICENSE_NUMBER : both map to the official 'certificate_license_number'.
    ('PASSPORT_NUMBER',       'GLINER', true,  0.80, 'IDENTITY', 'certificate_license_number', 'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    ...
    -- NATIONALITY : closest NVIDIA label is 'country'.
    ('NATIONALITY',           'GLINER', false, 0.80, 'IDENTITY', 'country',                    'LOW',    false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
...
(pii_type, detector, enabled, threshold, category, detector_label, severity, is_custom, created_at, updated_at, updated_by)
VALUES
    ('USERNAME',   'GLINER', true,  0.90, 'DIGITAL', 'user_name',   'LOW', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    -- ACCOUNT_ID : closest NVIDIA label is 'customer_id'.
...
(pii_type, detector, enabled, threshold, category, detector_label, severity, is_custom, created_at, updated_at, updated_by)
VALUES
    ('CREDIT_CARD_NUMBER',  'GLINER', true,  0.80, 'FINANCIAL', 'credit_debit_card',  'HIGH',   false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    ...
    ('TAX_ID',              'GLINER', true,  0.80, 'FINANCIAL', 'tax_id',             'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    -- SALARY : no NVIDIA equivalent ; placeholder mapping to 'unique_identifier' (entry stays disabled).
    ('SALARY',              'GLINER', false, 0.80, 'FINANCIAL', 'unique_identifier',  'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system')
    ON CONFLICT (pii_type, detector) DO NOTHING;
...
(pii_type, detector, enabled, threshold, category, detector_label, severity, is_custom, created_at, updated_at, updated_by)
VALUES
    -- AVS_NUMBER : Swiss AVS = national social-security identifier -> 'national_id'.
    ('AVS_NUMBER',             'GLINER', true, 0.80, 'MEDICAL', 'national_id',                    'HIGH',   false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    -- PATIENT_ID : closest NVIDIA label is 'unique_identifier'.
    ('PATIENT_ID',             'GLINER', true, 0.80, 'MEDICAL', 'unique_identifier',              'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    ...
    -- DIAGNOSIS / MEDICATION : no NVIDIA equivalent ; mapped to 'medical_record_number' (closest in domain).
    ('DIAGNOSIS',              'GLINER', true, 0.80, 'MEDICAL', 'medical_record_number',          'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    ('MEDICATION',             'GLINER', true, 0.80, 'MEDICAL', 'medical_record_number',          'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system')
    ON CONFLICT (pii_type, detector) DO NOTHING;
...
VALUES
    ...
    ('MAC_ADDRESS',  'GLINER', true,  0.80, 'IT_CREDENTIALS', 'mac_address',       'LOW',    false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    -- HOSTNAME : no NVIDIA equivalent ; mapped to 'url' (closest network identifier).
    ...
    ('SECRET_KEY',   'GLINER', false, 0.80, 'IT_CREDENTIALS', 'api_key',           'HIGH',   false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    -- SESSION_ID : session IDs are typically carried by HTTP cookies.
...
(pii_type, detector, enabled, threshold, category, detector_label, severity, is_custom, created_at, updated_at, updated_by)
VALUES
    -- CASE_NUMBER : closest NVIDIA label is 'unique_identifier'.
    ('CASE_NUMBER',             'GLINER', true, 0.80, 'LEGAL_ASSET', 'unique_identifier',          'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    ('LICENSE_NUMBER',          'GLINER', true, 0.80, 'LEGAL_ASSET', 'certificate_license_number', 'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    -- CRIMINAL_RECORD : no NVIDIA equivalent ; mapped to 'unique_identifier' as a placeholder.
    ('CRIMINAL_RECORD',         'GLINER', true, 0.80, 'LEGAL_ASSET', 'unique_identifier',          'HIGH',   false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    -- VEHICLE_REGISTRATION / VIN : both map to 'vehicle_identifier'.
    ...
    ('LICENSE_PLATE',           'GLINER', true, 0.90, 'LEGAL_ASSET', 'license_plate',              'LOW',    false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    ('VIN',                     'GLINER', true, 0.90, 'LEGAL_ASSET', 'vehicle_identifier',         'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system'),
    -- INSURANCE_POLICY_NUMBER : closest NVIDIA label is 'health_plan_beneficiary_number'.
    ('INSURANCE_POLICY_NUMBER', 'GLINER', true, 0.80, 'LEGAL_ASSET', 'health_plan_beneficiary_number', 'MEDIUM', false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'system')
    ON CONFLICT (pii_type, detector) DO NOTHING;
